...
- ВСЕ числовые значения с единицами измерения как ОТДЕЛЬНЫЕ термины:
  проценты ("10,5% годовых", "0,5 процентных пункта"),
  суммы в рублях ("5 000 000 рублей"),
  сроки ("5 рабочих дней", "6 месяцев", "20 лет"),
  количества ("25 процентов", "2 процентных пункта").
  Числовые значения извлекаются ОБЯЗАТЕЛЬНО — это критично для онтологии.

ВАЖНО — НОРМАЛИЗАЦИЯ ПАДЕЖА:
Возвращай термины в ИМЕНИТЕЛЬНОМ падеже единственного числа,
даже если в тексте они стоят в другом падеже:
- "кредитора" → "кредитор"
- "заёмщика" → "заёмщик"
- "овердрафта" → "овердрафт"
- "процентной ставки" → "процентная ставка"
- "лимита" → "лимит"
Исключение: устойчивые многословные термины оставляй как есть
("ключевая ставка ЦБ РФ", "договор поручительства").

НЕ ИЗВЛЕКАЙ:
- общеупотребительные слова без финансового смысла ("также", "является", "данный")
- предлоги, союзы, вводные слова
- усечённые формы: только полные термины ("кредит", а не "кред."; "правила кредитования", а не "правил")
